function back() {
  ...
  function updatePrice(itemNumber, quantityId, priceId) {
    // Special case for itemNumber 5 and 6
    if ( itemNumber === 6 || itemNumber === 5) {
      var quantity = parseInt(document.getElementById(quantityId).value);
  
      if ( quantity === 0 ) {
        // For the first quantity, set the price to 0
        document.getElementById(priceId).innerHTML = "&#8377; 0 ";
      } else {
        // For quantities greater than 1, each item costs Rs. 2500
        var total = 2500 * (quantity - 1);
